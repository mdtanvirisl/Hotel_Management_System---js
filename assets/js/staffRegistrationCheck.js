function validStaffRegistration(){
    ...
    if(flag == 9){
     ...
     if(flag == 10){
        if(!usertype){
            document.getElementById("usertypeError").innerHTML ="Please select your usertype.";
        }else{
           flag = 11;
        }
    }

    if(name != "" && email != "" && number != "" && address != "" && usertype != "" && gender != "" && username != "" && password != "" && confirmpassword != "" && flag == 11){
           
        register(userid, name, email, number, address, usertype, gender, username, password, confirmpassword);

    }
}

function register(userid, name, email, number, address, usertype, gender, username, password, confirmpassword){
    let xhttp = new XMLHttpRequest();
    xhttp.open("POST", "../controller/staffRegistration.php", true);
...
function searchStaff(){
    ...
    let xhttp = new XMLHttpRequest();
    xhttp.open('POST', '../controller/staffSearch.php', true);
    xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
    xhttp.send('search='+staffusername);
    xhttp.onreadystatechange = function(){
...
function displayResults(results) {
    ...
    if (results.length > 0) {
        ...
        headerRow.innerHTML = '<th>Id</th><th>Name</th><th>Email</th><th>Mobile Number</th><th>Gender</th><th>Username</th><th>Position</th><th>Address</th>';

        // rows with results
        for (let i = 0; i < results.length; i++) {
            let row = table.insertRow(i + 1);
            row.innerHTML = `<td>${results[i].StaffId}</td><td>${results[i].StaffName}</td><td>${results[i].StaffEmail}</td><td>${results[i].StaffNumber}</td><td>${results[i].Gender}</td><td>${results[i].StaffUserName}</td><td>${results[i].StaffType}</td><td>${results[i].StaffAddress}</td>`;
        }
    } else {
