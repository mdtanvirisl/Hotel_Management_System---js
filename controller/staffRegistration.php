<?php
    ...
    if(isset($_REQUEST['data'])){
        ...
        if(!empty($user->name) && !empty($user->email) && !empty($user->number) && !empty($user->gender) && !empty($user->address) && !empty($user->usertype) && !empty($user->username) && !empty($user->password) && !empty($user->confirmpassword) && $flag == 11){

            $user = [$user->userid, $user->name, $user->email, $user->number, $user->address, $user->usertype, $user->gender, $user->username, $user->password];

            echo json_encode(addstaff($user));
            addstaffuser($user);
            addsalarysheet($user);
        }
    }
