<?php
    ...
    if(isset($_REQUEST['data'])){
        ...
        if(!empty($user->name) && !empty($user->email) && !empty($user->number) && !empty($user->gender) && !empty($user->address) && $flag == 6){
            $user = [$user->name, $user->email, $user->number, $user->address, $user->gender, $user->username];

            echo json_encode(updatestaff($user));
            UpdateSalaryTableInformation($user);
        }
    }
?>